...
interface AirPurificationCardProps {
  imageSrc: string;
  imageAlt: string;
}

const AirPurificationCard: React.FC<AirPurificationCardProps> = ({ imageSrc, imageAlt }) => {
  const { t } = useLanguage();
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="p-6 md:p-8 md:w-2/3 flex flex-col justify-center">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-6 border-b border-blue-500/30 pb-4">
            {t("airPurification.title")}
          </h2>
          
          <p className="text-gray-200 leading-relaxed text-base md:text-lg">
            {t("airPurification.description")}
          </p>
          ...
          <div className="mt-6 pt-4 border-t border-white/10">
            <div className="flex flex-wrap gap-2">
              <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-900/40 text-blue-300 border border-blue-500/20">
                {t("airPurification.tags.highCapacity")}
              </span>
              <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-cyan-900/40 text-cyan-300 border border-cyan-500/20">
                {t("airPurification.tags.nanoTechnology")}
              </span>
              <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-900/40 text-indigo-300 border border-indigo-500/20">
                {t("airPurification.tags.oxygenBalance")}
              </span>
            </div>
          </div>
        </div>
...
export default AirPurificationCard;